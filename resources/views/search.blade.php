    ...
    <div class="feeds">
        <div class="user-container">
            <div class="d-flex flex-row align-items-center">
                ...
                <h3>@lang('search.show_result') {{ $search_req }}</h3>
            </div>
            <div style="margin-top: 10px;">
                <form class="d-flex flex-row" action="/search" method="get">
                    <input type=text name="search" placeholder="@lang('search.search_pc')" class="form-control" required>
                    <button style="width: 150px; margin-left: 10px;" type="submit" class="button-first">@lang('search.search')</button>
                </form>
            </div>
        </div>
        ...
        <div class="filter-container">
            ...
            <div style="margin-top: 10px;">
                <form class="d-flex flex-row" style="align-items: center;" action="/filter" method="get">
                    ...
                    <button style="width: 100px; margin-left: 10px;" type="submit" class="button-first">@lang('search.apply')</button>
                </form>
            </div>
        </div>
        <div class="filter-container">
            <h3 style="margin-bottom: 20px;">@lang('search.friends_list')</h3>
            @if(Auth::check())
                ...
                @empty
                    <p> @lang('search.no_friends') </p>
                @endforelse
            @else
                <p> @lang('search.please_login') </p>
            @endif
        </div>
    </div>
</div>
